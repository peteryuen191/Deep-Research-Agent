...
2. **Document Compilation (`docx` or `python-docx`)**: You MUST NOT just output Markdown as the final product. Write a Node.js script using the `docx` library (`npm install docx`) or a Python script using `python-docx` to programmatically build the final Word document. Python is heavily preferred on Linux hosts. (See `scripts/node/generate_docx.js` for a Node.js starter).
...
4. **Final Delivery**: Save the generated `.docx` file to the workspace output directory (e.g., `/root/.openclaw/workspace-main/output/research/Research_Report.docx`). Deliver this absolute path to the user.

### Phase 7: Knowledge Base & NotebookLM Integration
1. **Knowledge Base Update**: Synthesize the core findings, literature insights, and strategic takeaways. Write or append this synthesis directly to the workspace knowledge base (`/root/.openclaw/workspace-main/output/research/`) to maintain an ongoing, centralized knowledge base.
...
- **Linux Execution Context:** On Linux hosts, use `python3 script.py` to run scripts and `pip3 install` to install dependencies. Standard bash is used (no MSYS). For background async tasks and PDF extraction, Node.js (`node`) is preferred as it natively handles async loops well.
